        ...
        try:
            ...
            try:
                rc = await asyncio.wait_for(
                    _read_output(), timeout=timeout or drive_config.upload_timeout
                )
            except asyncio.TimeoutError:
                proc.kill()
                await proc.wait()
                logger.error(
                    f"rclone upload timeout after {timeout or drive_config.upload_timeout}s"
                )
                return False

            if rc != 0:
                logger.error(f"rclone upload failed, return code: {rc}")
                return False
    ...
    async def upload_file(
        ...
        if drive_config.upload_adapter == "rclone":
            ret = await CloudDrive.rclone_upload_file(
                ...
                save_path,
                local_file_path,
                timeout=drive_config.upload_timeout,
            )
            ...
            try:
                ...
                logger.error(
                    "aligo upload timeout after %ss", drive_config.upload_timeout
                )
                ret = False
