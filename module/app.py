...
from utils.format import parse_size_to_bytes, replace_date_time, validate_title
...
class Application:
    ...
    def __init__(
        ...
        self.forward_save_path = os.path.join(os.path.abspath("."), "forward")
        self.forward_temp_path = os.path.join(self.forward_save_path, "temp")
        self.api_id: str = ""
        self.api_hash: str = ""
        self.bot_token: str = ""
        self._chat_id: str = ""
        ...
        self.max_download_concurrent_transmissions: int = 1
        self.max_upload_concurrent_transmissions: int = 1
        self.web_host: str = "0.0.0.0"
        self.web_port: int = 5000
        self.max_download_task: int = 5
        self.max_upload_task: int = 1
        ...
        self.upload_queue_max_size: int = 0
        self.download_timeout: int = 900
        self.upload_timeout: int = 900
        self.flood_wait_extra: int = 5
        self.forward_max_size: int = 0
    ...
    def assign_config(self, _config: dict) -> bool:
        ...
        if _config.get("upload_drive"):
            ...
            if upload_drive_config.get("upload_timeout"):
                try:
                    self.cloud_drive_config.upload_timeout = int(
                        upload_drive_config.get("upload_timeout", 0)
                    )
        ...
        try:
            self.max_download_task = int(self.max_download_task)
        except (TypeError, ValueError):
            self.max_download_task = 1
        self.max_download_task = max(1, min(self.max_download_task, 3))
        ...
        self.max_download_concurrent_transmissions = get_config(
            _config,
            "max_download_concurrent_transmissions",
            self.max_concurrent_transmissions,
            int,
        )
        self.max_download_concurrent_transmissions = max(
            1, min(self.max_download_concurrent_transmissions, 3)
        )
        self.max_upload_concurrent_transmissions = get_config(
            _config,
            "max_upload_concurrent_transmissions",
            self.max_concurrent_transmissions,
            int,
        )
        self.max_upload_task = get_config(
            _config, "max_upload_task", self.max_upload_task, int
        )
        ...
        self.queue_max_size = get_config(
            _config, "queue_max_size", self.queue_max_size, int
        )
        self.upload_queue_max_size = get_config(
            _config, "upload_queue_max_size", self.upload_queue_max_size, int
        )
        self.download_timeout = get_config(
            _config, "download_timeout", self.download_timeout, int
        )
        self.upload_timeout = get_config(
            _config, "upload_timeout", self.upload_timeout, int
        )
        self.flood_wait_extra = get_config(
            _config, "flood_wait_extra", self.flood_wait_extra, int
        )
        forward_max_size = _config.get("forward_max_size", self.forward_max_size)
        parsed_forward_size = parse_size_to_bytes(forward_max_size)
        if parsed_forward_size is not None:
            self.forward_max_size = max(0, parsed_forward_size)
    ...
    async def upload_file(
        ...
        if self.cloud_drive_config.upload_adapter == "rclone":
            ...
            try:
                ...
                logger.error(
                    "aligo upload timeout after %ss",
                    self.upload_timeout or self.cloud_drive_config.upload_timeout,
                )
                ret = False
    ...
    def get_file_save_path(
        ...
        res: str = base_dir if base_dir else self.save_path
    ...
    def update_config(self, immediate: bool = True):
        ...
        self.config["max_concurrent_transmissions"] = self.max_concurrent_transmissions
        self.config["max_download_concurrent_transmissions"] = (
            self.max_download_concurrent_transmissions
        )
        self.config["max_upload_concurrent_transmissions"] = (
            self.max_upload_concurrent_transmissions
        )
        self.config["max_upload_task"] = self.max_upload_task
        self.config["queue_max_size"] = self.queue_max_size
        self.config["upload_queue_max_size"] = self.upload_queue_max_size
        self.config["download_timeout"] = self.download_timeout
        self.config["upload_timeout"] = (
            self.upload_timeout or self.cloud_drive_config.upload_timeout
        )
        self.config["flood_wait_extra"] = self.flood_wait_extra
        self.config["forward_max_size"] = self.forward_max_size
