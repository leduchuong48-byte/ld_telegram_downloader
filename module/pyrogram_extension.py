...
FILE_PART_MISSING = getattr(
    pyrogram.errors.exceptions.bad_request_400, "FilePartMissing", None
)
...
def _calc_backoff_seconds(base_seconds: float, attempt: int) -> float:
    """Calculate exponential backoff with jitter."""
    exp_seconds = base_seconds * (2 ** max(0, attempt - 1))
    jitter = random.uniform(0.0, max(1.0, base_seconds * 0.25))
    return exp_seconds + jitter


async def _run_with_timeout(coro, timeout: int, stop_fn=None, desc: str = ""):
    """Wrap coroutine with timeout and optional stop call"""
    try:
        return await asyncio.wait_for(coro, timeout=timeout)
    except asyncio.TimeoutError:
        if stop_fn:
            stop_fn()
        logger.error(f"{desc} timeout after {timeout}s")
        raise
...
async def upload_telegram_chat(
    ...
    if node.upload_telegram_chat_id:
        ...
        if download_status is DownloadStatus.SuccessDownload or (
            ...
            forward_status: ForwardStatus = ForwardStatus.FailedForward
            try:
...
async def upload_telegram_chat_message(
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            wait_seconds = _calc_backoff_seconds(
                wait_err.value + app.flood_wait_extra, attempt
            )
            await asyncio.sleep(wait_seconds)
            logger.warning(
                "Upload Message[{}]: FlowWait {}, sleep {:.1f}s",
                message.id,
                wait_err.value,
                wait_seconds,
            )
            ...
            if FILE_PART_MISSING and isinstance(e, FILE_PART_MISSING):
                ...
                wait_seconds = _calc_backoff_seconds(
                    max(1, app.flood_wait_extra), attempt
                )
                logger.warning(
                    "Upload Message[{}]: file part missing, retry in {:.1f}s",
                    message.id,
                    wait_seconds,
                )
                await asyncio.sleep(wait_seconds)
...
async def _upload_signal_message(
    ...
    if message.video:
        ...
        try:
            ...
            if node.reply_to_message:
                await _run_with_timeout(
                    node.reply_to_message.reply_video(
                        file_name,
                        caption=caption,
                    ...
                    timeout=app.upload_timeout,
                    stop_fn=upload_user.stop_transmission,
                    desc=f"Upload video {ui_file_name}",
                )
            else:
                await _run_with_timeout(
                    upload_user.send_video(
        ...
        if node.reply_to_message:
            await _run_with_timeout(
                node.reply_to_message.reply_photo(
                    file_name,
                    caption=caption,
                    message_thread_id=node.topic_id,
                ),
                timeout=app.upload_timeout,
                stop_fn=upload_user.stop_transmission,
                desc=f"Upload photo {ui_file_name}",
            )
        else:
            await _run_with_timeout(
                upload_user.send_photo(
                    upload_telegram_chat_id,
                    file_name,
                    caption=caption,
                    progress=update_upload_stat,
        ...
        if node.reply_to_message:
            await _run_with_timeout(
                node.reply_to_message.reply_document(
                    file_name,
                    caption=caption,
                    message_thread_id=node.topic_id,
                ),
                timeout=app.upload_timeout,
                stop_fn=upload_user.stop_transmission,
                desc=f"Upload document {ui_file_name}",
            )
        else:
            await _run_with_timeout(
                upload_user.send_document(
                    upload_telegram_chat_id,
                    file_name,
                    caption=caption,
                    progress=update_upload_stat,
        ...
        if node.reply_to_message:
            await _run_with_timeout(
                node.reply_to_message.reply_voice(
                    file_name,
                    caption=caption,
                    message_thread_id=node.topic_id,
                ),
                timeout=app.upload_timeout,
                stop_fn=upload_user.stop_transmission,
                desc=f"Upload voice {ui_file_name}",
            )
        else:
            await _run_with_timeout(
                upload_user.send_voice(
                    upload_telegram_chat_id,
                    file_name,
                    caption=caption,
                    progress=update_upload_stat,
        ...
        if node.reply_to_message:
            await _run_with_timeout(
                node.reply_to_message.reply_video_note(
                    file_name,
                    caption=caption,
                    message_thread_id=node.topic_id,
                ),
                timeout=app.upload_timeout,
                stop_fn=upload_user.stop_transmission,
                desc=f"Upload video note {ui_file_name}",
            )
        else:
            await _run_with_timeout(
                upload_user.send_video_note(
                    upload_telegram_chat_id,
                    file_name,
                    caption=caption,
                    progress=update_upload_stat,
...
def _reset_media_group_cache(node: TaskNode, media_group_id: Optional[str]):
    """Reset cached media group to force re-upload."""
    if not media_group_id:
        return
    group = node.media_group_ids.get(media_group_id)
    if not group:
        return
...
async def forward_multi_media(
    ...
    if not node.media_group_ids[message.media_group_id][message.id]:
        ...
        try:
            ...
            if isinstance(e, FLOOD_WAIT_EXCEPTIONS):
                logger.warning(
                    "Upload Message[{}]: FlowWait {}",
                    message.id,
            ...
            if FILE_PART_MISSING and isinstance(e, FILE_PART_MISSING):
                _reset_media_group_cache(node, message.media_group_id)
                logger.warning(
                    "Upload Message[{}]: file part missing, reset media group cache",
                    message.id,
                )
                raise
...
async def proc_cache_forward(
    ...
    if node.reply_to_message:
        ...
        business_connection_id = getattr(
            node.reply_to_message, "business_connection_id", None
        )
...
def set_max_concurrent_transmissions(
    client: pyrogram.Client,
    max_download_transmissions: int,
    max_upload_transmissions: Optional[int] = None,
):
    """Set maximum concurrent transmissions"""
    if max_upload_transmissions is None:
        max_upload_transmissions = max_download_transmissions
    max_download_transmissions = max(1, int(max_download_transmissions))
    max_upload_transmissions = max(1, int(max_upload_transmissions))
    if getattr(client, "max_concurrent_transmissions", None):
        client.max_concurrent_transmissions = max(
            max_download_transmissions, max_upload_transmissions
        )
        client.save_file_semaphore = asyncio.Semaphore(max_upload_transmissions)
        client.get_file_semaphore = asyncio.Semaphore(max_download_transmissions)
