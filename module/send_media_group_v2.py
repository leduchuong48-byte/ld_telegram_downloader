...
def _get_reply_to(
    ...
    if hasattr(utils, "get_reply_to"):
        return utils.get_reply_to(
            message_thread_id=message_thread_id,
            reply_to_message_id=reply_to_message_id,
        )
    if not message_thread_id and not reply_to_message_id:
        return None
...
async def send_media_group_v2(
    ...
    see pyrogram
    """
    invoke_kwargs = {"sleep_threshold": 60}
    if business_connection_id is not None:
        invoke_kwargs["business_connection_id"] = business_connection_id
    ...
    try:
        ...
        if "business_connection_id" in str(exc):
            invoke_kwargs.pop("business_connection_id", None)
            log.warning(
                "business_connection_id not supported by this pyrogram version, retrying without it"
            )
            r = await client.invoke(
                raw.functions.messages.SendMultiMedia(
                    peer=await client.resolve_peer(chat_id),
