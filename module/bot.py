...
async def send_help_str(client: pyrogram.Client, chat_id):
    ...
    msg = (
        ...
        f"/cleanup on|off - {_t('Delete downloaded files after forward/upload')}\n"
        f"/forward-clean - Clean forward folders\n"
        f"/forward-limit SIZE|off - Set forward folder size limit\n"
...
async def toggle_cleanup(client: pyrogram.Client, message: pyrogram.types.Message):
    """切换转发后是否删除本地文件"""
    ...
    _bot.app.after_upload_telegram_delete = enabled
    _bot.app.config["after_upload_telegram_delete"] = enabled
    await client.send_message(
        message.from_user.id,
        _t("Cleanup after upload is turned on")
        if enabled
        else _t("Cleanup after upload is turned off"),
    )


async def forward_clean(client: pyrogram.Client, message: pyrogram.types.Message):
    """清理 forward 与 forward/temp"""
    forward_root = _bot.app.forward_save_path
    forward_temp = _bot.app.forward_temp_path
    os.makedirs(forward_root, exist_ok=True)
    freed = clear_dir_contents(forward_root)
    os.makedirs(forward_temp, exist_ok=True)
    await client.send_message(
        message.from_user.id,
        f"forward 清理完成，释放 {format_byte(freed)}",
    )


async def set_forward_limit(client: pyrogram.Client, message: pyrogram.types.Message):
    """设置 forward 目录容量上限"""
    args = message.text.split()
    if len(args) != 2:
        await client.send_message(
            message.from_user.id,
            "参数错误，请使用 /forward-limit SIZE|off，例如 /forward-limit 20GB",
        )
        return

    value = args[1].lower()
    if value in ("off", "0", "disable", "none"):
        _bot.app.forward_max_size = 0
        _bot.app.config["forward_max_size"] = 0
        await client.send_message(message.from_user.id, "已关闭 forward 容量上限")
        return

    size_bytes = parse_size_to_bytes(args[1])
    ...
    _bot.app.forward_max_size = size_bytes
    _bot.app.config["forward_max_size"] = size_bytes

    os.makedirs(_bot.app.forward_save_path, exist_ok=True)
    total_size = get_dir_size(_bot.app.forward_save_path)
    freed = 0
    if total_size >= size_bytes:
        freed = cleanup_dir_by_freeing(
            _bot.app.forward_save_path, int(size_bytes * 0.3)
        )

    await client.send_message(
        message.from_user.id,
        (
            f"forward 上限已设置为 {format_byte(size_bytes)}，"
            f"当前占用 {format_byte(total_size)}，"
            f"本次清理 {format_byte(freed)}"
        ),
    )
...
async def stop(client: pyrogram.Client, message: pyrogram.types.Message):
    ...
    cleanup_paths = []
    if _bot.app:
        cleanup_paths.append(os.path.join(_bot.app.save_path, "temp"))
    ...
    if removed_files or removed_dirs:
        await client.send_message(
            message.chat.id,
            f"{_t('Stop')} - cleaned temp files: {removed_files}, dirs: {removed_dirs}",
        )
