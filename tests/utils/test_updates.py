...
class FakeHTTPSResponse:
    ...
    def read(self):
        if self.status == 200:
            return b'{"name":"v0.0.0 2022-03-02","tag_name":"v0.0.0", "html_url":"https://github.com/example/ld_tg_downloader/releases/tag/v0.0.0"}'
        else:
            return b"{error}"
...
class TestUpdates(unittest.TestCase):
    @patch("utils.updates.GITHUB_REPO", "example/ld_tg_downloader")
    @patch("requests.get")
    def test_get_latest_release(self, mock_get):
        ...
        mock_response.text = json.dumps(
            {
                ...
                "tag_name": "v0.0.0",
                "html_url": "https://github.com/example/ld_tg_downloader/releases/tag/v0.0.0",
            }
        )
        ...
        self.assertEqual(
            result["html_url"],
            "https://github.com/example/ld_tg_downloader/releases/tag/v0.0.0",
        )
